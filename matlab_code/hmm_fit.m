...
%
    ...
    if strcmpi(experiment, 'sim')
        filename = [mfilename, '_sim'];
    else
        filename = mfilename;
    end
    fname = fullfile(fdir, sprintf('%s.mat', filename)); 
    if ~exist(fname, 'file')
    ...
    if ~exist(fname, 'file')
        ...
        parameters = nan(N, 4*number_of_parameters);
...
%
    ...
    transformed_parameters = [vol, sto];
    ...
    loglikelihood = log(predictions + eps) .* choices + log(1 - predictions + eps) .* (1 - choices);
